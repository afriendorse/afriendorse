...
import 'package:afriendorse/feature/suggest_new_service/model/suggest_service_model.dart';
import 'package:afriendorse/feature/suggest_new_service/repository/suggest_service_repo.dart';
...
class SuggestServiceController extends GetxController {
  ...
  Future<void> getSuggestedServiceList(int offset, {reload = false}) async {
    ...
    Response response = await suggestServiceRepo.getSuggestedServiceList(
      offset,
    );
    if (response.statusCode == 200) {
      suggestedServiceModel = SuggestedServiceModel.fromJson(response.body);
      if (offset != 1) {
        suggestedServiceList.addAll(
          suggestedServiceModel!.content!.suggestedServiceList!,
        );
      } else {
  ...
  Future<void> submitNewServiceRequest() async {
    ...
    if (response.statusCode == 200) {
      clearData();
      customSnackBar(
        "service_request_placed_successfully_to_admin".tr,
