...
class WalletController extends GetxController {
  ...
  Future<void> getWalletTransactionData(
    int offset, {
    bool reload = false,
    String type = "",
  }) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (offset != 1) {
        listOfTransaction!.addAll(
          walletTransactionModel!.content!.transactions!.data!,
        );
      } else {
        listOfTransaction = [];
        listOfTransaction!.addAll(
  ...
  // Add this method inside WalletController class
  Future<bool> deductAndRefresh({
    required double amount,
    required String purpose,
  }) async {
    final newBalance = await walletRepo.deductWalletFunds(
      amount: amount,
      purpose: purpose,
    );
    ...
    // Patch local balance immediately so UI reflects change at once
    if (walletTransactionModel?.content != null) {
      walletTransactionModel!.content!.walletBalance = newBalance;
      update();
    }

    // Reload full history in background so debit row appears in list
    unawaited(getWalletTransactionData(1, reload: true));
  ...
  Future<double> getWalletBalance() async {
    ...
    if (walletTransactionModel?.content?.walletBalance != null) {
      return walletTransactionModel!.content!.walletBalance!;
    }

    // Otherwise fetch fresh data
    await getWalletTransactionData(1);
    return walletTransactionModel?.content?.walletBalance ?? 0.0;
  }
  ...
  void insertFilterList() {
    ...
    for (int i = 0; i < AppConstants.walletTransactionSortingList.length; i++) {
      _walletFilterList.add(
        WalletFilterBody.fromJson(AppConstants.walletTransactionSortingList[i]),
      );
    }
  }
