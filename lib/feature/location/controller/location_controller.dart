...
class LocationController extends GetxController implements GetxService {
  ...
  Future<ZoneResponseModel> getZone(
    ...
    // 🔍 1. PRINT THE EXACT COORDINATES BEING SENT
    print('🌍 [DEBUG] Checking Zone for Lat: $lat, Lng: $long');

    ZoneResponseModel responseModel;
    Response response = await locationRepo.getZone(lat, long);

    // 🔍 2. PRINT THE API RESPONSE
    print('🌍 [DEBUG] Zone API Status: ${response.statusCode}');
    print('🌍 [DEBUG] Zone API Body: ${response.body}');
    ...
    if (response.statusCode == 200 && response.body != null) {
      // Safely handle if the server returns a String instead of a JSON Map
      ...
      if (response.body is Map) {
        ...
        try {
          bodyMap = jsonDecode(response.body);
        } catch (e) {
          print('🌍 [DEBUG] Error decoding API body: $e');
        }
      }

      // Check if 'content' exists and is not null
      if (bodyMap != null && bodyMap['content'] != null) {
        isSuccess = true;
        _inZone = true;

        // Safely extract Zone ID
        if (bodyMap['content']['zone'] != null) {
          currentZoneId = bodyMap['content']['zone']['id']?.toString() ?? '';
          _zoneID = currentZoneId;
        }

        // Safely extract available services count
        if (bodyMap['content']['available_services_count'] != null) {
          totalServiceCountInZone =
              int.tryParse(
                bodyMap['content']['available_services_count'].toString(),
              ) ??
              0;
        }
        ...
        isSuccess = false;
        _inZone = false;
        message = bodyMap?['message'] ?? 'Service not available in this area';
      }
    } else {
      isSuccess = false;
      _inZone = false;

      // Safely handle error message from server
    ...
    print('🌍 [DEBUG] Is in Zone? $_inZone | Message: $message');
  ...
  Future<List<PredictionModel>> searchLocation(
    ...
    if (text.isNotEmpty) {
      try {
        ...
        if (response.statusCode == 200 && response.body != null) {
          ...
          // Handle case where content is a List (empty results) or Map (with suggestions)
          if (content is Map && content.containsKey('suggestions')) {
            var suggestions = content['suggestions'];
            if (suggestions is List) {
              _predictionList = suggestions
                  .map((prediction) => PredictionModel.fromJson(prediction))
                  .toList()
                  .cast<PredictionModel>();
            }
          } else if (content is List && content.isEmpty) {
            // Empty results - no suggestions
    ...
    print('🔍 Returning ${_predictionList.length} predictions');
    update();
