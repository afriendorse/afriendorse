...
class VerticalScrollableTabViewState extends State<VerticalScrollableTabView>
  ...
  SliverList buildVerticalSliverList() {
    return SliverList(
      delegate: SliverChildListDelegate(
        List.generate(widget._listItemData.length, (index) {
  ...
  bool onScrollNotification(ScrollNotification notification) {
    ...
    // ✅ Guard: bail out if not enough items are visible yet
    ...
    if (notification is UserScrollNotification) {
      if (categoryCount == 2) {
        if (visibleItems.isEmpty) return false; // ✅ guard
        if (notification.metrics.pixels <= 5) {
          widget._tabController.animateTo(visibleItems[0]);
        } else {
          if (visibleItems.length < 2) return false; // ✅ guard
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          ...
          if (visibleItems.isEmpty) return; // ✅ guard

          if (currentPixels <= 5) {
            widget._tabController.animateTo(visibleItems[0]);
          } else {
            if (visibleItems.length < 2) return; // ✅ guard
