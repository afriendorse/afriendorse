...
import 'package:afriendorse/helper/debounce_helper.dart';
import 'package:afriendorse/util/core_export.dart';
import 'package:afriendorse/feature/booking/model/service_availability_model.dart';
import 'package:afriendorse/feature/booking/widget/provider_available_bottom_sheet.dart';
import 'package:afriendorse/feature/booking/widget/service_unavailable_dialog.dart';
import 'package:get/get.dart';

// enum BookingStatusTabs { all, pending, accepted, ongoing, completed, canceled }
enum BookingStatusTabs { pending, accepted, ongoing, completed, canceled }
...
class ServiceBookingController extends GetxController implements GetxService {
  ...
  // FIX #3: Default is .pending (matches the screen's initState intent).
  BookingStatusTabs _selectedBookingStatus = BookingStatusTabs.pending;
  ...
  // FIX #3: New method — sets the selected tab in state and triggers a UI
  // rebuild WITHOUT making any API call and WITHOUT the early-return guard.
  // Use this from initState when you just want to synchronise the visual
  // tab highlight with the data fetch you are already issuing separately.
  void setSelectedTabSilently(BookingStatusTabs tab) {
    _selectedBookingStatus = tab;
    update();
  }

  // FIX #3: updateBookingStatusTabs retains the early-return guard so that
  // tapping the already-active tab from the UI is a no-op (good UX). The
  // guard no longer causes problems on init because we use
  // setSelectedTabSilently there instead.
  void updateBookingStatusTabs(
    ...
    if (_selectedBookingStatus == bookingStatusTabs) return;

    _selectedBookingStatus = bookingStatusTabs;
    update(); // Update UI immediately to reflect selected tab.
  ...
  void updateSelectedServiceType({ServiceType? type}) {
    if (type != null) {
      ...
      // No update() call needed here since nothing visible changed yet;
      // the caller is responsible for triggering a fetch if required.
    }
  }
