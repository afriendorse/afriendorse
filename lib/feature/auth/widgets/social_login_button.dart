...
import 'package:afriendorse/feature/auth/widgets/existing_account_bottom_sheet.dart';
...
class SocialLoginButton extends StatelessWidget {
    ...
    if (isRoute) {
      if (token != null) {
        final bool canRedirectToRedirectUrl =
            redirectUrl != null && redirectUrl != RouteHelper.initial;

        if (canRedirectToRedirectUrl) {
          // ==========================================
          // FIX: Safely parse the redirectUrl if it's JSON
          // ==========================================
          String finalRedirectRoute = redirectUrl!;

          if (finalRedirectRoute.startsWith('{') &&
              finalRedirectRoute.contains('redirect_to')) {
            try {
              final Map<String, dynamic> decoded = jsonDecode(
                finalRedirectRoute,
              );
              finalRedirectRoute = decoded['redirect_to'] ?? finalRedirectRoute;
            } catch (e) {
              debugPrint("Error parsing redirectUrl JSON: $e");
            }
          }

          Get.offAllNamed(finalRedirectRoute);
          // ==========================================
        } else {
  ...
  Future<void> _onTap({required SocialLoginType socialLoginType}) async {
    if (socialLoginType == SocialLoginType.google) {
      final SocialLogInBody? socialLoginModel = await Get.find<AuthController>()
          .googleLogin();
