...
class PriceConverter {
  ...
  static String convertPrice(
    ...
    double formattedPrice = defaultPrice; // ✅ use a safe local copy

    if (discount != null && discountType != null) {
      if (discountType == 'amount') {
        formattedPrice = formattedPrice - discount;
        if (formattedPrice < 0) formattedPrice = 0; // ✅ cleaner null-safe guard
      } else if (discountType == 'percent') {
        formattedPrice = formattedPrice - ((discount / 100) * formattedPrice);
      }
    }
    ...
    // ✅ Extracted formatting to a reusable spot
    final String formattedNumber = formattedPrice
    ...
    final String result = isRightSide
        ? '$formattedNumber${getCurrency()}'
        : '${getCurrency()}$formattedNumber';

    return isShowLongPrice ? result : longToShortPrice(result);
  }
