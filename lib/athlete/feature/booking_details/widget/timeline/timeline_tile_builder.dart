...
import 'package:afriendorse/athlete/feature/booking_details/widget/timeline/timeline_node.dart';
import 'package:afriendorse/athlete/feature/booking_details/widget/timeline/timeline_theme.dart';
import 'package:afriendorse/athlete/feature/booking_details/widget/timeline/timeline_tile.dart';
...
typedef ConnectedConnectorBuilder =
    Widget? Function(BuildContext context, int index, ConnectorType type);
...
class TimelineTileBuilder {
    ...
    return TimelineTileBuilder(
      ...
      startConnectorBuilder: _createConnectedStartConnectorBuilder(
        ...
        connectorBuilder: (context, index, __) => _createStyledConnectorBuilder(
          connectorStyleBuilder?.call(context, index),
        )(context),
      ),
      endConnectorBuilder: _createConnectedEndConnectorBuilder(
        connectionDirection: connectionDirection,
  ...
  factory TimelineTileBuilder({
    ...
    return TimelineTileBuilder._((context, index) {
      final tile = TimelineTile(
        mainAxisExtent: itemExtent ?? itemExtentBuilder?.call(context, index),
        node: TimelineNode(
          indicator:
              indicatorBuilder?.call(context, index) ?? Indicator.transparent(),
      ...
      if (theme != null) {
        return TimelineTheme(data: theme, child: tile);
      } else {
        return tile;
      }
    }, itemCount: itemCount);
  ...
  const TimelineTileBuilder._(this._builder, {required this.itemCount})
    : assert(itemCount >= 0);
